...
| API key 模型 | 每个 API/tool 使用独立环境变量 |
...
- 配置注册表：把每个 MCP tool 映射到其独立的 API key 环境变量。
...
- 工具本地响应归一化层：由各个 tool（或共享 tool base）完成字段归一化，同时保留 `raw`。
- 限流防护层：处理可重试的超限和超时错误。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `get_longtail_keywords_5118` | 海量长尾词挖掘 v2 / `/keyword/word/v2` | `API_5118_LONGTAIL_V2` | 同步 | `keyword` | `pageIndex`、`pageSize`、`sortField`、`sortType`、`filter`、`filterDate` | `keywords[]` | `pageSize <= 100`；支持排序和快速筛选 |
| `get_industry_frequency_words_5118` | 细分行业分析 / `/tradeseg` | `API_5118_FREQ_WORDS` | 同步 | `keyword` | 无 | `frequencyWords[]` | 返回高频分词及占比 |
| `get_suggest_terms_5118` | 下拉联想词挖掘 / `/suggest/list` | `API_5118_SUGGEST` | 同步 | `word`、`platform` | 无 | `suggestions[]` | `platform` 必须是文档列出的厂商取值之一 |
| `get_keyword_metrics_5118` | 关键词搜索量信息 v2 / `/keywordparam/v2` | `API_5118_KW_PARAM_V2` | 异步 | `keywords[]` | 共享异步控制字段 | `items[]` | 最多 50 个关键词；厂商使用 `\|` 分隔；处理中可能返回 `101` |
...
| `get_mobile_traffic_keywords_5118` | 移动流量词挖掘 / `/traffic` | `API_5118_TRAFFIC` | 异步 | `keyword` | 共享异步控制字段、分页参数 | `keywords[]` | 长耗时任务；第二步必须同时传 `taskId` 和 `keyword` |

### 排名词导出

| MCP 工具 | 官方 API / endpoint | 环境变量 | 模式 | 必填输入 | 关键可选参数 | 归一化数据字段 | 约束与说明 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `get_pc_site_rank_keywords_5118` | PC 网站排名词导出 v2 / `/keyword/pc/v2` | `API_5118_BAIDUPC_V2` | 同步 | `url` | `pageIndex` | `items[]` | 厂商结果根字段是 `data.baidupc[]`；`pageSize` 固定 500 |
...
| `get_mobile_site_rank_keywords_5118` | 移动网站排名词导出 v2 / `/keyword/mobile/v2` | `API_5118_MOBILE_V2` | 同步 | `url` | `pageIndex` | `items[]` | 厂商结果根字段是 `data.baidumobile[]`；`pageSize` 固定 500 |
| `get_domain_rank_keywords_5118` | PC 整站排名词导出 v2 / `/keyword/domain/v2` | `API_5118_DOMAIN_V2` | 同步 | `url` | `pageIndex` | `items[]` | 面向整域聚合；适配器只暴露 `pageIndex`；厂商结果根字段是 `data.domain[]`；返回分页大小为 `1000` |

### 竞价情报

| MCP 工具 | 官方 API / endpoint | 环境变量 | 模式 | 必填输入 | 关键可选参数 | 归一化数据字段 | 约束与说明 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `get_bid_sites_5118` | 竞价推广公司挖掘 / `/bidsite` | `API_5118_BIDSITE` | 同步 | `keyword` | `pageIndex`、`pageSize`、`includeHighlight` | `items[]` | 竞价公司情报查询；分页大小最多 500 |
| `get_bid_keywords_5118` | 网站竞价词 v2 / `/bidword/v2` | `API_5118_BIDWORD_V2` | 同步 | `url` | `pageIndex`、`pageSize`、`includeHighlight` | `items[]` | 竞价词导出；分页大小最多 500 |
...
| MCP 工具 | 官方 API / endpoint | 环境变量 | 模式 | 必填输入 | 关键可选参数 | 归一化数据字段 | 约束与说明 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `check_url_indexing_5118` | URL 收录检测 / `/include` | `API_5118_INCLUDE` | 异步 | `urls[]` | 共享异步控制字段 | `items[]` | 最多 200 个 URL；完成信号依赖 `check_status` |

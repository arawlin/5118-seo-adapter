...
同一个 MCP 进程内，所有出站请求共享同一个全局限流队列。

可选的 MCP 请求控制环境变量：

- `MCP_5118_MIN_TIME_MS`（默认 `1500`）
- `MCP_5118_MAX_CONCURRENT`（默认 `1`）
- `MCP_5118_RESERVOIR`（默认 `1`）
- `MCP_5118_MAX_RETRIES`（默认 `2`）
- `MCP_5118_BASE_BACKOFF_MS`（默认 `1200`）
- `MCP_5118_MAX_BACKOFF_MS`（默认 `5000`）
- `MCP_5118_JITTER_MS`（默认 `500`）
